...
def search_domain(domain, index):
    record_list = []
    print(f"[*] Trying target domain: {domain}")
    print(f"[*] Trying index: {index}")

    cc_url = f"http://index.commoncrawl.org/CC-MAIN-{index}-index?url={domain}&matchType=domain&output=json"
    print("cc_url:", cc_url)
    ...
    try:
        response = session.get(cc_url, timeout=30, stream=True)
        ...
        if response.status_code == 200:
            ...
            print(f"[*] Added {len(record_list)} results.")
        else:
...
initial_record_list = search_domain(domain, index_list)
